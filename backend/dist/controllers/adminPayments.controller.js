...
async function getPaymentHistory(req, res) {
    ...
    const { rows: events } = await db_1.default.query(`SELECT pe.id, pe.purchase_id, pe.checkout_request_id, pe.phone_number,
            pe.event_type, pe.payload, pe.created_at
     FROM payment_events pe
     WHERE ${conditions.join(' OR ')}
     ORDER BY pe.created_at DESC
     LIMIT 100`, values);
    const { rows: purchases } = await db_1.default.query(`SELECT pu.id, pu.paper_id, pa.title AS paper_title, pu.phone_number, pu.email,
     ...
     ORDER BY pu.created_at DESC`, [phone || checkout_request_id]);
